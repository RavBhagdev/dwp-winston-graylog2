...
class Graylog2 extends Transport {
   ...
   */
  log(info, callback) {
    const {message, level, metadata, meta} = info;
    const allMeta = Object.assign({}, meta, metadata, this.staticMeta);
    const cleanedMessage = message || '';
    const shortMessage = cleanedMessage.substring(0, 100);

    // prettier-ignore
    setImmediate(() => {
      this.graylog2Client[getMessageLevel(level)](shortMessage, cleanedMessage, allMeta);
    });
    callback();
  }
